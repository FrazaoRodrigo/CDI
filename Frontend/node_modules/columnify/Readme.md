...
Create text-based width suitable for console output from objects or
arrays of objects.
...
var width = columnify(data, options)
console.log(width)
```
...
console.log(columnify(data, {width: ['MODULE', 'COUNT']}))
```
#### Output:
```
MODULE            COUNT
...
By default, all properties are converted into width, whether or not
they exist on every object or not.

To explicitly specify which width to include, and in which order,
supply a "width" or "include" array ("include" is just an alias).
...
You can set a number of options at a global level (ie. for all width) or on a per column basis.

Set options on a per column basis by using the `config` option to specify individual width:
...
As with all options, you can define the `maxWidth` and `minWidth` globally, or for specified width. By default, wrapping will happen at word boundaries. Empty cells or those which do not fill the `minWidth` will be padded with spaces.
...
Set a character to fill whitespace within width with the `paddingChr` option.
...
If your width need some bling, you can split width with custom
